# Data2Seq.py - Versión actualizada
import sys
...
class Data2Seq(nn.Module):
    ...
    def __init__(self, modality, dim, **kwargs):
        super().__init__()
        ...
        if self.modality == 'image' or self.modality == 'infrared' or self.modality == 'x-ray':
            ...
            embed_type = kwargs.get('embed_type', 'separate')  # 'single' o 'separate'
            
            if num_features is None:
                raise ValueError("num_features must be specified for tabular modality")
            
            if embed_type == 'single':
                # Todo el vector como un solo token
                self.embed = Tabular.TabularEmbed(
                    num_features=num_features,
                    embed_dim=self.embed_dim
                )
            else:
                # Cada feature como token separado
                self.embed = Tabular.TabularEmbedSeparateTokens(
                    num_features=num_features,
                    embed_dim=self.embed_dim
                )
    ...
    def get_audio_embeddings(self, audio):
        ...
        audio_embedding = torchaudio.compliance.kaldi.fbank(
            waveform1, htk_compat=True, sample_frequency=sr, 
    ...
    def forward(self, data):
        if self.modality in ['image', 'infrared', 'x-ray', 'video', 'graph', 'hyper', 'time-series', 'imu']:
